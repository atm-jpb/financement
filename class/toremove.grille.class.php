...
class TFin_grille_leaser extends TObjetStd {
	function __construct($type='LEASER') { /* declaration */
		...
		parent::set_table(MAIN_DB_PREFIX.'fin_grille');
		parent::add_champs('fk_type_contrat,type','type=chaine;index;');
		parent::add_champs('fk_soc','type=entier;index;');//fk_soc_leaser
		
			//	parent::add_champs('nb_ligne,nb_colonne','type=entier;');//fk_soc_leaser
		
		parent::add_champs('TGrille','type=tableau;');
	...
	function loadByContratLeaser(&$ATMdb, $idLeaser, $idTypeContrat) {
		
		$ATMdb->Execute("SELECT rowid FROM ".$this->get_table()." WHERE fk_soc=".$idLeaser." AND fk_type_contrat='".$idTypeContrat."' LIMIT 1");
     ...
     */
    function get_grille(&$ATMdb, $periodicite='TRIMESTRE' , $options=array(),$type='LEASER')
    {
