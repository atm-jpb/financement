...
class TFin_dossier extends TObjetStd {
	function __construct() { /* declaration */
		...
		parent::add_champs('fk_statut_renta_neg_ano,fk_statut_renta_neg_spe,fk_statut_dossier',array('type'=>'chaine'));
...
//echo $fact->paye;
	...
	function load_statut_dossier() {
		...
		// Statut renta neg spécifique
		$sql = 'SELECT code, label FROM '.MAIN_DB_PREFIX.'c_financement_statut_renta_neg_spe WHERE entity IN (0, '.$conf->entity.') AND active = 1';
		$resql = $db->query($sql);
		...
		if ($resql)
		{
			while ($row = $db->fetch_object($resql))
			{
				$this->TStatutRentaNegSpe[$row->code] = $row->label;
			}
		}
	}
}
