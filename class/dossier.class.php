...
class TFin_dossier extends TObjetStd {
	...
	function generate_factures_leaser($paid = false, $delete_all = false) {
		...
		$cpt = 0;
		while($f->date_prochaine_echeance < time() && $f->date_prochaine_echeance < $f->date_solde  && $f->numero_prochaine_echeance <= $f->duree && $cpt<50) { // On ne créé la facture que si l'échéance est passée et qu'il en reste
