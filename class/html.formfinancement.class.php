...
/**
 *	\file       htdocs/core/class/html.FormFinancement.class.php
 *  \ingroup    core
...
class FormFinancement
{
	...
	/**
	 *  Return combo list with const from financement module
	 *
	 *  @param  string	$type		Type of const needed
	...
	function select_financement($type, $selected='',$htmlname='const_id')
	{
		...
		$sql = "SELECT rowid, code FROM ".MAIN_DB_PREFIX."fin_const";
		$sql.= " WHERE type = '".$type."'";
		$sql.= " AND active = 1";
	 ...
	 *  @return	void
	 */
	function array_financement($type)
	{
		global $conf,$langs,$user;
		...
		if ($resql)
		{
			...
			if ($num)
			{
				...
				{
					$obj = $this->db->fetch_object($resql);
					$out[$obj->rowid] = ($langs->trans($obj->code)!=$obj->code ? $langs->trans($obj->code) : $obj->code);
					$i++;
				}
			}
		}
