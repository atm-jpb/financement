...
select d.rowid,
    ...
    lea.nom  as leaser,
    d.display_solde = 0 as rule1,
    d.montant >= (select coalesce(c.value, 50000) from ".MAIN_DB_PREFIX."const c where c.name = 'FINANCEMENT_MAX_AMOUNT_TO_SHOW_SOLDE' and c.entity in (0, 1, d.entity) order by c.entity desc limit 1) as rule2,
    d.soldepersodispo = 2 as rule3,
    (case d.nature_financement
    ...
    (select count(ee.fk_source)
     ...
     where ee.fk_source = d.rowid and f.paye = 0) > (select c4.value from ".MAIN_DB_PREFIX."const c4 where name = 'FINANCEMENT_NB_INVOICE_UNPAID' and c4.entity in (0, 1, d.entity) order by c4.entity desc limit 1) as rule7
...
while($obj = $db->fetch_object($resql)) {
    ...
    for($i = 1 ; $i <= 7 ; $i++) $TData[] = $obj->{'rule'.$i};
