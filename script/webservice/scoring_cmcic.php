    ...
    //					if ($simulationSuivi->leaser->array_options['options_edi_leaser'] == $fuser->array_options['options_fk_leaser_webservice'])
				...
				if ($found)
				{
					...
					if (empty($error))
					{
						...
						if ($statut == 'Status.APPROVED' && ($simulation->accord != 'OK' || ($simulationSuivi->statut_demande == 2 && $simulationSuivi->statut != 'OK'))) $simulationSuivi->doAction($PDOdb, $simulation, 'accepter');
						else if ($statut == 'Status.REJECTED' && ($simulation->accord != 'OK' || ($simulationSuivi->statut_demande == 2 && $simulationSuivi->statut != 'KO'))) $simulationSuivi->doAction($PDOdb, $simulation, 'refuser');
